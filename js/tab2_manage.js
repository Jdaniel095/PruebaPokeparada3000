...
// ✅ NUEVA FUNCIÓN PARA MANEJAR EL TOGGLE DEL FORMULARIO
function setupFormToggle() {
  const container = document.getElementById("tab-content");
  if (!container) return;

  // 1. Busca el botón y el contenido del formulario
  // Asumo que el botón tiene data-target="#form-gestion-content"
  // y el contenido tiene id="form-gestion-content"
  const toggleBtn = container.querySelector(
    ".toggle-btn[data-target='#form-gestion-content']"
  );
  const content = document.getElementById("form-gestion-content");

  if (!toggleBtn || !content) {
    // Si no los encuentra, no hace nada.
    return;
  }

  // 2. Asegura el estado inicial (minimizado por defecto)
  ...
  // 3. Agrega el listener DIRECTO al botón
...
// Tiempo máximo que vive la cache (5 minutos)
...
async function cargarTarjetas(forceRefresh = false) {
  ...
  try {
    ...
    if (data.ok && Array.isArray(data.propuestas)) {
      saveToLocalCache(data.propuestas); // ✅ Guardamos cache
      renderTarjetas(data.propuestas);   // ✅ Pintamos tarjetas
    } else {
...
function renderTarjetas(lista) {
  ...
  lista.forEach((p) => {
    propuestasCache.set(p.id, p); // guardo para edición
    html += generarHtmlDeTarjeta(p); // 👈 usa tu función existente
  });
...
/* ==============================
   ✅ Al guardar nueva propuesta o editar
...
async function enviarPropuestaActual(btn) {
  ...
  try {
    ...
    saveToLocalCache(null);  // ⬅ limpiar cache
    cargarTarjetas(true);    // ⬅ cargar de servidor
...
if (!window.google || !window.google.maps) {
  ...
  async function fetchGeocode(coordInput) {
    ...
    if (!geocoder) {
      try {
        // 1. Llama a la función de main.js y espera
        await loadGoogleMapsApi(); 
        
        // 2. Inicializa el geocoder (esta línea se perdió en el refactor)
    ...
    // --- El resto de tu función (esto ya estaba bien) ---
...
// === PARTE 4/4 ===
  ...
  if (e.target.checked) {
    ...
    // ✅ Ahora con Promise en vez de callback
